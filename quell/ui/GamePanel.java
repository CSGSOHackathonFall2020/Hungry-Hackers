...
import static api.CellState.CLOSED_GATE;
import static api.CellState.OPEN_GATE;
import static api.CellState.PEARL;
import static api.CellState.PORTAL;
import static api.CellState.isSpikes;
...
public class GamePanel extends JPanel
{
     ...
     */
    public GamePanel(CS227Quell game, JPanel scorePanel)
    {
        this.game = game;
    ...
    public void paintComponent(Graphics g)
    {
        ...
        if (animating && animations != null && animations.size() > 0)
        {
            ...
            Cell last = descriptors[descriptors.length - 1].getCell();
            boolean playerDead = isSpikes(last.getState()) && descriptors[0].getNewIndex() == descriptors.length - 1;

            for (int i = 0; i < descriptors.length; ++i)
            {
                ExtendedDescriptor ed = descriptors[i];
                CellState s = ed.getCell().getState();
...
//    }
    ...
    private void drawCellPixelCoords(Graphics g, int row, int col, Cell c, double percent)
    {
        ...
        if (!animating && game.isOver() && s == CellState.EMPTY)
        {
        ...
        if (CellState.isSpikes(s) || CellState.isMovable(s))
        {
            g.setColor(Color.BLACK);
     ...
     * @return
     */
    private ExtendedDescriptor[] setCellSequence(int startingRow, int startingCol, Direction d, Descriptor[] descriptors)
    {
        ...
        for (int i = 0; i < ret.length; ++i)
        {
            ...
            doPortalJump = ret[i].getCell().getState() == PORTAL && !doPortalJump;
     ...
     * @return
     */
    private boolean sanityCheck(Descriptor[] predescriptors, Cell[][] beforeGrid, int startingRow, int startingCol, Direction dir)
    {
...
//    }
        ...
        for (int i = 0; i < descriptors.length; ++i)
        {
            ...
            Cell found = d.getCell();
            Cell expected = beforeGrid[d.getOldRow()][d.getOldCol()];
            if (found.getState() != expected.getState())
            {
                ...
                String msg2 = "Cell sequence in direction " + dir + " starting at (" + startingRow + ", " + startingCol +
                        ...
                        ",  found " + found.getState() + " in descriptor. ";
                log("ERROR: " + msg + msg2);
                return false;
            }
        }
        ...
        for (int i = 1; i < descriptors.length; ++i)
        {
            ...
            if (i != newIndex && !d.isDisappeared())
            {
                ...
                if (expected.getState() != found.getState() )
                {
                    if (!(expected.getState() == OPEN_GATE && found.getState() == CLOSED_GATE))
                    {
                        ...
                        String msg2 = "Cell sequence in direction " + dir + " starting at (" + startingRow + ", " + startingCol +
                                ") should have cell (" + d.getOldRow() + ", " + d.getOldCol() + ") with state " + expected.getState() + " at index " +  i +
                                " moved to (" + newRow + ", " + newCol + ") with state " + expected.getState() + " at index " +  newIndex + ", " +
        ...
        {
            if (predescriptors[i].getCell().getState() == PORTAL)
            {
                if (i >= newPlayerPosition || predescriptors[i + 1].getCell().getState() != PORTAL)
                {
            ...
            if (CellState.isMovable(predescriptors[i].getCell().getState()))
            {
        ...
        {
            if (predescriptors[i].getCell().getState() == PORTAL && i < predescriptors.length - 1)
            {
     ...
     * @param dir
     */
    private void setUpAnimation(Descriptor[] predescriptors, int startingRow, int startingCol, Direction dir)
    {
        int newPlayerPosition = predescriptors[0].getMovedTo();
        ...
        while (i < descriptors.length - 1)
        {
            ...
            if (descriptors[i].getCell().getState() == PORTAL)
            {
        ...
        for (int j = 0; j < descriptors.length - 1; ++j)
        {
            ...
            if (CellState.isMovable(exd.getCell().getState())) // && exd.getNewIndex() != exd.getIndex())
            {
                // in first segment where we found the movable block,
                // should be visible until moved
                boolean first = true;

                CellState blockState = exd.getCell().getState();
                mblockCount += 1;
        ...
        for (int j = 0; j < descriptors.length - 1; ++j)
        {
            ...
            else if (exd.getCell().getState() == OPEN_GATE)
            {
                // make an animation step that will just become non visible after end time
                int startRow = exd.getOldRow();
                int startCol = exd.getOldCol();
            ...
            else if (CellState.isMovable(exd.getCell().getState()))
            {
                mblockCount += 1;
            }
        }
        ...
        Cell c = exd.getCell();
        if (CellState.isSpikes(c.getState()) && descriptors[0].getNewIndex() == descriptors.length - 1)
        {
            int startRow = exd.getOldRow();
            int startCol = exd.getOldCol();
            int endRow = startRow;
    ...
    private class MyKeyListener implements KeyListener
    {
        ...
        public void keyPressed(KeyEvent event)
        {
            ...
            // do the move
            Descriptor[] predescriptors = game.move(dir);
